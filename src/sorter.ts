...
type FmKF = { fmKey: CfgFmKey[keyof CfgFmKey], fmFbk: CfgFmFbk[keyof CfgFmFbk] }
...
export default class Sorter {
    ...
    constructor(fileExpView: FileExplorerView, userConfig: NwtCfg, statusBarEl: HTMLElement, statusBarSpan: HTMLElement) {
    ...
    sort() {
        ...
        if (false === (this.fileExpView.ready && this.fileExpView.containerEl.isShown()))
            return
    ...
    private cacheFmDataInFolder(folder: TFolder) {
        ...
        for (const absFile of folder.children) {
            ...
            if (fmDataInFolder[absFile.path] !== undefined)
                continue;
    ...
    private getSortedItemsAndSetIcon(folder: TFolder) {
        ...
        for (const absFile of absFileArray) {
            ...
            this.setIconByFm(item, 'leaf', 'nwt-headless', Boolean(fmDataInFolder[absFile.path][this.userConfig.key_headless_md]))

            // retitled, only available in folder
            if (absFile instanceof TFolder) {
                this.setIconByFm(item, 'text-cursor-input', 'nwt-retitled', Boolean(fmDataInFolder[absFile.path][this.userConfig.key_retitled]))
            }
    ...
    private setIconByFm(item: Item, iconName: string, iconContainerClsName: string, desiringIcon: boolean) {
        ...
        // hasIcon is true, but desiringIcon is false, remove svg 
            ...
            const container = selfEl.createDiv({ cls: ['nav-file-tag', iconContainerClsName] })
